...
/**
 * Loads a JavaScript configuration from a plugin.
 * @param {string} filePath The plugin name to load.
 * @returns {Object} The configration object from the plugin.
 * @throws {Error} If the package cannot be read.
 * @private
 */
function loadPluginConfig(filePath) {
    var parts = filePath.split("/"),
        pluginName = parts[0],
        configName = parts[1];
    debug("Loading config plugin: " + filePath);
    try {
        if (!configName) {
            debug("Error reading package: " + filePath);
            throw new Error("No configuration name specified");
        }
        var plugin = require(pluginName);
        if (!plugin.configs) {
            debug("Error reading package: " + filePath);
            throw new Error("Plugin \"" + pluginName + "\" does not expose any configs");
        }
        if (configName && !plugin.configs[configName]) {
            debug("Error reading package: " + filePath);
            throw new Error("Plugin \"" + pluginName + "\" is missing \"" + configName + "\" configuration");
        }
        return plugin.configs[configName];
    } catch (e) {
        debug("Error reading package: " + filePath);
        e.message = "Cannot read config \"" + configName + "\" from plugin: \"" + filePath + "\"\nError: " + e.message;
        throw e;
    }
}
...
function loadConfigFile(filePath) {
    ...
    } else if (filePath.indexOf("eslint-plugin") >= 0) {
        config = loadPluginConfig(filePath);
    } else {
...
function resolve(filePath) {
    ...
    if (isFilePath(filePath)) {
        ...
        // it's a package
        return filePath.indexOf("plugin:") === 0 ? normalizePackageName(filePath.split(":")[1], "eslint-plugin") : normalizePackageName(filePath, "eslint-config");
    }
}
